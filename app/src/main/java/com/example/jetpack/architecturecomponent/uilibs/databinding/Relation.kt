...
@BindingMethods(
    value = [
        //1.1 对调不成功
        BindingMethod(
            type = MyTextView::class, attribute = "text", method = "setBackground"
        ...
        //1.3.
        BindingMethod(type = MyTextView::class, attribute = "android:alpha", method = "setAlpha1"),
        //1.4.
        BindingMethod(
            type = MyTextView::class, attribute = "cameraDistance", method = "setCameraDistance"
        )
//        ,
//        BindingMethod(
//            type = android.widget.ImageView::class, attribute = "app:tint", method = "setImageTintList"
//        )
    ]
)
...
//    }//note:对子类不起作用, 如果双向绑定使用了android:text 这个方法会导致异常getTextBeforeCursor on inactive InputConnection 原因不明
    ...
    fun convertColorToDrawable(color: String): Drawable {
        Log.i(TAG, "convertColorToDrawable------------------${color}")
    ...
    @JvmStatic
    fun setListeners(view: MyButton, attrChange: InverseBindingListener) {
        view.setOnClickListener {//view的事件----引起view属性更改事件----调用@InverseBindingAdapter------把值传递给变量
//            if (view.myName == "123") {
            Log.i(TAG, "setOnClickListener----更新")
//
//            } else {
//
//            }
    ...
    fun setMyName(view: MyButton, str: String?) {
        str?.apply {
            if (view.myName != str) {//如果新值和旧值不一样才赋值
                Log.i(TAG, "set myName-----${str}")
                view.myName = str
            }
        }
    }

    const val TAG = "Relation"
}