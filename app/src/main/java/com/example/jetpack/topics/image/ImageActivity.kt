 ...
 * 2. 可绘制对象列表
 *    [各种Drawable](https://developer.android.google.cn/guide/topics/resources/drawable-resource)
 * 3. 向可绘制对象添加色调
 * 4. 从Bitmap中提取颜色
 *    [Palette API 选择颜色](https://developer.android.google.cn/training/material/palette-colors)
 * 5. VectorDrawable 矢量可绘制对象
 *     在 XML 文件中定义为一组点、线条和曲线及其相关颜色信息。使用矢量可绘制对象的主要优势在于图片可缩放。您可以在不降低显示质量的情况下缩放图片，
 *     也就是说，可以针对不同的屏幕密度调整同一文件的大小，而不会降低图片质量 <vector>
 *     5.1 创建VectorDrawable
 ...
 *     5.2 AnimatedVectorDrawable
 *         为VectorDrawable添加动画[矢量图动画](https://developer.android.google.cn/guide/topics/graphics/drawable-animation)
