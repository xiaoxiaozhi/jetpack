...
/**
 * 在 Android 11 下测试
 * https://developer.android.google.cn/guide/components/activities/tasks-and-back-stack?hl=en#Starting 中文版还是20年没有更新
 * 任务与Activity堆栈
 ...
 * 4. 定义启动模式
 *    两种方式定义不同启动模式清单中定义(manifest)和在Intent的flag中定义，如果两种方法同时启动一个Activity，应该以Intent为准
 *    4.1 清单中<Activity>定义不同的启动模式
 *    在<Activity>中定义的启动模式 ：
 *    standard:默认启动模式 每次启动Activity就会创建新的实例，每个实例可以属于不同的任务
 *    singleTop:如果已经在栈顶则会调用 onNewIntent() 方法来将 intent 转送给该实例，如果不再栈顶创建Activity实例
 *    singleTask:如果活动属于自己任务： 如果不存在则创建新任务新实例，如果存在则清空实例之上的Activity，并调用nonNewIntent()；如果活动属于其他任务，则其他任务的堆栈将置于自己之上。详情请看onNewIntent注释
 *    singleInstance:每次都创建新堆栈和新实例，堆栈中只能有这一个实例
 *    singleInstancePerTask：android12中添加的
...
//            startActivity(Intent(this@TaskBackStackActivity, SingleInstanceActivity::class.java))
     ...
     *   onCreate--->onStart--->onResume---onPause--->(实例存在，然后收到启动模式为singleTop的调用)onNewIntent--->onResume
     *   如果不在栈顶，则会创建实例
     ...
     *   onCreate--->onStart--->onResume---onPause--->onStop--->onStart--->(实例存在，然后收到启动模式为singleTask的调用)onNewIntent--->onResume
